...
const Cart = (props) => {
  ...
  return (
    <div className="text-center bg-dark text-warning p-1">
      ...
      <h5>
        Enrolled Courses: <span className="text-white">{course.length}</span>
      </h5>
      {course.map((course) => (
        <div className="d-flex">
          <p className="mr-auto p-2">{course.title}</p>
          <h6 className="text-white p-2">{course.price} EGP</h6>
        </div>
      ))}
      <hr className="bg-danger" />
      <div className="d-flex">
        <h4 className="mr-auto p-2">Total :</h4>
        <h6 className="text-white p-2">{totalPrice} EGP</h6>
      </div>
    </div>
  );
};
